...
Page({
  data: {
    ...
    loading: false,
    showModal: false,
    form: { deviceName: '', version: '', productId: '' }
  },
  ...
  onAdd() {
    this.setData({ showModal: true, form: { deviceName: '', version: '', productId: this.data.productId } });
  },
  ...
  async onSubmit() {
    ...
    if (!form.deviceName || !form.version) { showToast('请填写完整信息'); return; }
    showLoading();
    try {
